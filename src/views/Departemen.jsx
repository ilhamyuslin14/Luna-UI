...
const INITIAL_DATA = [
  { name: 'HR',          lowongan: 'Backend Engineer',  tanggal: '19 Feb 2026', canDelete: true },
  { name: 'Engineering', lowongan: 'Frontend Developer', tanggal: '19 Feb 2026', canDelete: false },
  { name: 'Marketing',   lowongan: 'Social Media Lead',  tanggal: '19 Feb 2026', canDelete: false },
  { name: 'Finance',     lowongan: 'Head of Finance',    tanggal: '19 Feb 2026', canDelete: true },
  { name: 'Product',     lowongan: 'UX Researcher',      tanggal: '19 Feb 2026', canDelete: true },
];
      ...
      <div className="dept-body">
          ...
          <div className="dept-right-actions">
            ...
            {selectedRows.size > 0 && (
              <div className="dept-bulk-container">
                <button className={`dept-btn-bulk${showBulkDropdown ? ' active' : ''}`} onClick={() => setShowBulkDropdown(v => !v)}>
                ...
                {showBulkDropdown && (
                  <div className="dept-bulk-dropdown active">
                    <a href="#" className="bulk-dropdown-item" onClick={(e) => { e.preventDefault(); handleBulkArchive(); }}>
                      <ArchiveSvg /> Arsipkan
                    </a>
                  </div>
                )}
            ...
            <div className="dept-filter-container">
              <button className={`dept-btn-filter${showFilterDropdown ? ' active' : ''}`} onClick={() => setShowFilterDropdown(v => !v)}>
                <img src="/assets/line240.svg" /> Filter
              </button>
              {showFilterDropdown && (
                <div className="dept-filter-dropdown active" style={{ padding: '12px', minWidth: '130px' }}>
                  <div style={{ textTransform: 'uppercase', fontSize: '11px', fontWeight: '700', color: '#a0a7b5', marginBottom: '12px', letterSpacing: '0.5px' }}>Status</div>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {['Aktif', 'Arsip'].map((status) => (
                      <div key={status} style={{ display: 'flex', flexDirection: 'column' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: '600', color: '#171e2c', paddingBottom: '8px' }}>
                          <input type="checkbox" style={{ width: '15px', height: '15px', accentColor: '#0977be', cursor: 'pointer', borderRadius: '4px', border: '1px solid #cbd0db' }} />
                          {status}
                        </label>
                        <div style={{ height: '1px', background: '#f0f2f6', marginBottom: '8px' }}></div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="dept-table-container">
          <table className="dept-table">
            <thead>
              <tr>
                <th width="24"><input type="checkbox" className="dept-checkbox-all" checked={selectAll} onChange={toggleSelectAll} /></th>
                  ...
                  <td>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                      ...
                      <div className="dept-actions">
                        ...
                        {dept.canDelete ? (
                          <button className="dept-btn-outline btn-delete" onClick={() => alert(`${dept.name} berhasil dihapus!`)}>
                            <DeleteSvg /> Delete
                          </button>
                        ) : (
        ...
        <div className="dept-modal-overlay" onClick={() => setArchiveTarget(null)}>
          <div className="dept-modal dept-modal-confirm" onClick={e => e.stopPropagation()}>
            <div className="dept-modal-confirm-text">
              ...
              <p className="dept-modal-subtitle">Apakah Anda yakin ingin mengarsipkan departemen ini?</p>
            </div>
