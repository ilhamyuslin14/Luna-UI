...
import { generateCVParsingOpenAI } from '../../utils/generateCVParsingOpenAI';
...
const EMPTY_KANDIDAT = {
  ...
  email: '', phone: '', industri: '', tahun_terakhir_bekerja: '', harapanUpah: '', harapanBenefit: '',
  skills: [], pengalaman_kerja: [], pendidikan: [], sertifikasi: [],
};
...
export default function SandboxCVParsing({ navigate }) {
  ...
  // Mapping ke state form KandidatRingkasan. KandidatRingkasan membaca
  // skills/pengalaman_kerja/pendidikan/sertifikasi langsung dari dalam
  // object `kandidat` (bukan prop terpisah), jadi semuanya digabung di sini.
  const [parsedKandidat, setParsedKandidat] = useState(EMPTY_KANDIDAT);

  // ── Riwayat CV Parsing (dari ai_usage_history, diisi tiap kali generate) ──
  ...
  const fetchHistory = async (page = historyPage) => {
    ...
    try {
      ...
      const { data, error, count } = await supabase
        ...
        .eq('function_name', 'Generate CV Parsing')
      ...
      console.error('Gagal mengambil riwayat CV Parsing:', err);
    } finally {
  ...
  // Ambil config AI terbaru dari DB. Dipanggil baik saat mount (untuk tampilan awal)
  // maupun tepat sebelum generate (karena semua tab Sandbox dimount sekaligus dan
  // tidak pernah unmount saat pindah tab — kalau cuma di-fetch sekali di mount,
  // perubahan provider yang disimpan lewat tab Konfigurasi setelah CV Parsing tab
  // ini terbuka tidak akan pernah ke-pick up).
  ...
  // KandidatRingkasan membaca skills/pengalaman_kerja/pendidikan/sertifikasi
  // langsung dari dalam object `kandidat` (bukan prop terpisah), jadi semuanya
  // digabung di sini. Dipakai baik setelah generate baru maupun saat memuat
  // ulang JSON dari Riwayat, supaya kedua alur konsisten.
  const applyParsedDataToPreview = (parsedData) => {
    ...
    setParsedKandidat(prev => ({
      ...
      tahun_terakhir_bekerja: dt.tahun_terakhir_bekerja ? String(dt.tahun_terakhir_bekerja) : '',
  ...
  const handleSelectHistory = (row) => {
    ...
    applyParsedDataToPreview(row.output_json);
  };
  ...
  const handleGenerateParsing = async () => {
    ...
    // Selalu re-fetch config di sini (bukan pakai state hasil mount) supaya
    // provider/model yang baru saja disimpan di tab Konfigurasi selalu terpakai.
    ...
    try {
      const generateFn = liveConfig.provider === 'openai' ? generateCVParsingOpenAI : generateCVParsing;
      ...
      applyParsedDataToPreview(result.parsedData);

      // Insert ke ai_usage_history di generateCVParsing(OpenAI) adalah fire-and-forget,
    ...
    <div className="flex flex-col h-full bg-white/50 backdrop-blur animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between px-8 py-6 border-b border-slate-200/60 bg-white/40">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">CV Parsing</h1>
            <p className="text-sm text-slate-500 mt-1 max-w-lg">Unggah CV kandidat untuk diekstrak dan ditampilkan ke profil kandidat secara otomatis.</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-8 py-6 border-b border-slate-200/60">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-sm font-bold text-slate-800">Riwayat CV Parsing</h2>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-500 text-[11px] uppercase tracking-wide">
              <tr>
                ...
                <th className="text-left px-4 py-2.5 font-semibold">Kandidat</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {historyList.map(row => {
                const nama = row.output_json?.detail_kandidat?.nama_lengkap;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-slate-800 font-medium">{nama || <span className="text-slate-400 italic font-normal">Tidak diketahui</span>}</td>
              ...
              {historyList.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-6 text-center text-slate-400 text-sm">
                    {loadingHistory ? 'Memuat riwayat...' : 'Belum ada riwayat CV Parsing.'}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="p-8 border-b border-slate-200/60">
        ...
        <div className="bg-white border border-slate-200/60 rounded-2xl p-6 shadow-sm mb-6">
          <p className="text-sm font-semibold text-slate-700 mb-4">Unggah CV kandidat untuk diekstrak secara otomatis</p>
          <div>
        ...
        <div className="flex flex-col gap-1.5 mt-8">
          <label className="text-sm font-semibold text-slate-700">Output JSON CV Parsing</label>
          <textarea
      ...
      <div className="bg-slate-50/50 pb-8">
        <div className="flex items-center gap-3 px-8 py-5">
          ...
          <span className="text-xs font-medium text-slate-400">Profil Kandidat (Sandbox View)</span>
        </div>
        {/* Render ulang dengan parameter unique key agar trigger mount ulang saat data berubah */}
        <KandidatRingkasan
          key={JSON.stringify(parsedKandidat)}
