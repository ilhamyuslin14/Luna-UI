    ...
    <div className="sd-view" onClick={() => setShowStatusDrop(false)}>
      ...
      <div className="sd-title-bar">
        ...
        <div className="sd-title-actions">
          <button className="sd-header-btn-primary" onClick={() => navigate('kandidat-tambah')}>
      ...
      {(pageCreation === 'creating' || showSuccess) && (
        <div className={`sd-toast${showSuccess ? ' sd-toast--success' : ''}`}>
          {pageCreation === 'creating' && (
            <>
              <div className="sd-toast-row">
                <div className="sd-creation-spinner" />
                <div className="sd-toast-text">
                  <span className="sd-toast-label">Membuat laman karir...</span>
                  <span className="sd-toast-sub">Harap tunggu sebentar</span>
                </div>
                <span className="sd-toast-pct">{Math.round(creationProgress)}%</span>
              </div>
              <div className="sd-creation-track">
                <div className="sd-creation-fill" style={{ width: `${creationProgress}%` }} />
              </div>
            </>
          )}
          {showSuccess && (
            <div className="sd-toast-row">
              ...
              <span className="sd-toast-label" style={{ color: '#089f32' }}>Laman karir berhasil dibuat!</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="sd-subnav">
        <div className="sd-tabs">
          <button className={`sd-tab${activeTab === 'kandidat' ? ' active' : ''}`} onClick={() => onTabChange('kandidat')}>Kandidat</button>
          <button className={`sd-tab${activeTab === 'ringkasan' ? ' active' : ''}`} onClick={() => onTabChange('ringkasan')}>Ringkasan</button>
        </div>
      </div>

      <div className="sd-content">
        <button className="sd-back-btn" onClick={() => back ? back() : navigate('seleksi')} style={{ marginBottom: 4 }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {activeTab === 'kandidat' ? (
          <SeleksiKandidat navigate={navigate} />
        ) : (
