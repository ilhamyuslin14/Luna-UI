        ...
        <div className="kan-right-actions">
          ...
          {selectedRows.size > 0 && (
            <div className="kan-bulk-container">
              <button className={`kan-btn-bulk${showBulkDropdown ? ' active' : ''}`} onClick={() => setShowBulkDropdown(v => !v)}>
              ...
              {showBulkDropdown && (
                <div className="kan-bulk-dropdown active">
                  <a href="#" className="bulk-dropdown-item" onClick={(e) => { e.preventDefault(); alert(`${selectedRows.size} Kandidat berhasil ditambahkan ke Lowongan!`); setSelectedRows(new Set()); setShowBulkDropdown(false); }}>
                  ...
                  <div className="kan-bulk-dropdown-divider"></div>
                  <a href="#" className="bulk-dropdown-item" onClick={(e) => { e.preventDefault(); const n = selectedRows.size; setShowBulkDropdown(false); setArchiveModal({ title: 'Arsipkan Kandidat', body: `Apakah Anda yakin ingin mengarsipkan ${n} kandidat yang dipilih?`, onConfirm: () => { setSelectedRows(new Set()); setArchiveModal(null); } }); }}>
                    <ArchiveSvg /> Arsipkan
                  </a>
                </div>
              )}
          ...
          <div className="kan-filter-container">
            <button className={`kan-btn-filter${showFilterDropdown ? ' active' : ''}`} onClick={() => setShowFilterDropdown(v => !v)}>
            ...
            {showFilterDropdown && (
              <div className="kan-filter-dropdown active">
                <div className="kan-filter-column w-left">
                  ...
                  {['Aktif','Arsip'].map(s => <div key={s}><div className="kan-filter-item"><input type="checkbox" className="kan-filter-checkbox" /><label>{s}</label></div><div className="kan-filter-divider-horizontal"></div></div>)}
                  <div className="kan-filter-divider-horizontal-thick"></div>
                  <span className="kan-filter-column-title">Pengalaman</span>
                  {['0-2 Tahun','2-5 tahun','5-10 tahun','>10 tahun'].map(s => <div key={s}><div className="kan-filter-item"><input type="checkbox" className="kan-filter-checkbox" /><label>{s}</label></div><div className="kan-filter-divider-horizontal"></div></div>)}
                </div>
                ...
                <div className="kan-filter-column w-right">
                  ...
                  {['Intern','Junior','Staff','Senior','Supervisor','Manager','Head of','General Manager','Advisor'].map(s => <div key={s}><div className="kan-filter-item"><input type="checkbox" className="kan-filter-checkbox" /><label>{s}</label></div><div className="kan-filter-divider-horizontal"></div></div>)}
                </div>
              </div>
            )}
          </div>
