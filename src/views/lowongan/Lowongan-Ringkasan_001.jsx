...
import { getSeleksiById, updateSeleksi as updateSeleksiRaw } from '../../services/seleksiService.js';
...
const EditableContent = memo(
  ({ htmlRef, initialHtml }) => (
    <div
      ...
      className="sd001-deskripsi-content sd001-deskripsi-editable"
...
const DESKRIPSI_HTML = `<h3 class="sd001-deskripsi-section-title">Role Overview</h3>
<p class="sd001-deskripsi-p">We are looking for a proactive and results-driven Talent Acquisition Specialist to join our HR team. You will be responsible for the full lifecycle of recruitment, from sourcing and interviewing to closing top-tier candidates.</p>
<h4 class="sd001-deskripsi-subtitle">Key Responsibilities</h4>
<ul class="sd001-deskripsi-list">
...
<h4 class="sd001-deskripsi-subtitle">Requirements &amp; Qualifications</h4>
<ul class="sd001-deskripsi-list">
...
export default function LowonganRingkasan_001({ seleksiId, jabatan = 'Project Manager', navigate }) {
  ...
  // Satu sumber kebenaran buat resolve type+options field, dipakai oleh
  // mode edit satuan (InlineEditRow) maupun mode edit-all (renderValue),
  // biar keduanya selalu konsisten.
  const resolveField = (field) => {
    ...
    if (field.type === 'date') return { type: 'date', options: [] };
    if (field.key === 'jumlah' || field.key === 'pengalaman') return { type: 'number', options: [] };
    if (field.type === 'location') return { type: 'location', options: [] };
  ...
  const renderValue = (field) => {
    ...
    // Resolve type/options sama persis kayak mode edit satuan (InlineEditRow)
    const { type, options } = resolveField(field);

    // ── Edit mode ──
    if (type === 'dropdown') {
      const optList = field.key === 'dept'
        ? departments.map(d => ({ value: d.id, label: d.name }))
        : (options || []).map(opt => ({ value: opt, label: opt }));
      return (
        <div className="sd001-detail-select-wrap">
          <select
            className="sd001-detail-select"
            value={val}
            onChange={e => handleEditChange(field.key, e.target.value)}
          >
            <option value="" disabled>Tambahkan data</option>
            {optList.map(o => (
    ...
    if (type === 'date') {
      return (
        <input
          type="date"
          className="sd001-detail-input sd001-detail-input--date"
          value={val}
          onChange={e => handleEditChange(field.key, e.target.value)}
        />
      );
  ...
  return (
    <div className="sd001-columns">
      {/* Kolom Kiri */}
      <div className="sd001-col-left">
        <div className="sd001-card">
          <div className="sd001-card-header">
            <span className="sd001-card-title">Detail Pekerjaan</span>
            {!isEditing && (
              <button className="sd001-edit-btn" onClick={() => setIsEditing(true)}>
                <EditIcon /> Edit
              </button>
            )}
          </div>
          <div className="sd001-detail-rows">
            {FIELDS.map(field => {
              if (!isEditing && field.type !== 'readonly') {
                // Determine props for InlineEditRow (sumbernya sama kayak renderValue)
                const { type, options } = resolveField(field);
          ...
          {isEditing && (
            <div className="sd001-detail-edit-footer">
              <button className="sd001-edit-cancel-btn" onClick={handleCancel}>Batal</button>
              <button className="sd001-edit-save-btn" onClick={handleSave}>Simpan</button>
            </div>
          )}
        </div>
        ...
        <div className="sd001-card">
          <div className="sd001-card-header">
            <span className="sd001-card-title">Deskripsi Pekerjaan</span>
            {!isEditingDeskripsi && (
              <button className="sd001-edit-btn" onClick={() => setIsEditingDeskripsi(true)}>
                <EditIcon /> Edit
              </button>
            )}
          </div>
          ...
          {isEditingDeskripsi ? (
            <div className="sd001-deskripsi-editor">
              <div className="sd001-deskripsi-toolbar">
                <select
              ...
              <div className="sd001-detail-edit-footer">
                <button className="sd001-edit-cancel-btn" onClick={handleCancelDeskripsi}>Batal</button>
                <button className="sd001-edit-save-btn" onClick={handleSaveDeskripsi}>Simpan</button>
              </div>
            </div>
          ) : (
            <div
              className="sd001-deskripsi-content"
