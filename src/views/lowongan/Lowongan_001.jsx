...
export default function Lowongan_001({ navigate, searchQuery = '' }) {
  ...
  } = useLowonganData(companyId, companyPlan, { searchQuery });
  ...
  const handleShare = (platform, row) => {
    handleShareBase(platform, row, companyName);
  ...
  // board view (kanban) — fitur presentasi khusus desktop, dibangun dari rows/alur
  // milik hook, jadi ikut update tiap kali hook selesai fetch atau filter berubah
  useEffect(() => {
    const boardRows = applyStatusFilters(rows, activeFilters);
    setBoardColumns(buildBoardFromData(alurList, boardRows, maxAlurMap));
  }, [rows, alurList, maxAlurMap, activeFilters]);
                        ...
                        <div className="lw001-status-wrapper" onClick={(e) => { e.stopPropagation(); setOpenStatusIdx(openStatusIdx === row.id ? null : row.id); }}>
                          ...
                          {openStatusIdx === row.id && (
                            <div className="lw001-status-dropdown active">
                              {statusConfigEntries.map(([key, s]) => (
                                <div key={key} className="lw001-status-dropdown-item" data-status={key} onClick={(e) => { e.stopPropagation(); handleUpdateStatus(row.id, key); }}>
