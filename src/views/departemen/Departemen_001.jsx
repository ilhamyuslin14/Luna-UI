...
export default function Departemen_001({ navigate, searchQuery = '' }) {
  ...
  const closeAddModal = () => {
    if (closingAddModal) return;
    setClosingAddModal(true);
    setTimeout(() => { setShowAddModal(false); setClosingAddModal(false); }, 180);
  };
  ...
  return (
    <div className="dept001-view" onClick={(e) => {
      ...
      <div className="dept001-body">
        ...
        <div className="dept001-table-container">
          <table className="dept001-table">
            <thead>
              <tr>
                <th width="3%"><input type="checkbox" className="dept001-checkbox-all" checked={selectAll} onChange={toggleSelectAll} disabled={pagedData.length === 0 || loading} /></th>
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={dept.id} className={dept.status === 'arsip' ? 'dept001-row-archived' : ''}>
                    <td><input type="checkbox" className="dept001-checkbox row-checkbox" checked={selectedRows.has(dept.id)} onChange={() => toggleRow(dept.id)} /></td>
                    <td className="dept001-name" onClick={() => navigate && navigate('departemen-detail_001', { departemen: dept.name, departemenId: dept.id })} style={{ cursor: 'pointer' }}>{dept.name}</td>
                    <td>
                      <span style={{ background: 'var(--luna-orange-050)', color: 'var(--luna-orange-500)', padding: '4px 12px', borderRadius: '16px', fontWeight: '600', fontSize: '11px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                        {dept.totalPosisi ?? 0}
                      </span>
                    </td>
                    <td>{dept.contact || '-'}</td>
                    <td>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                        ...
                        <div className="dept001-actions">
                          {dept.status === 'arsip' ? (
                            <button className="dept001-btn-outline" onClick={() => setUnarchiveTarget(dept)}>
                              <UnarchiveSvg /> Tampilkan
                            </button>
                          ) : (
                            <button className="dept001-btn-outline btn-archive" onClick={() => setArchiveTarget(dept)}>
                              <ArchiveSvg /> Arsipkan
                            </button>
                          )}
                        </div>
      ...
      {showAddModal && (
        <div className={`dept001-modal-overlay${closingAddModal ? ' dept001-closing' : ''}`} onClick={closeAddModal}>
            ...
            <div className="dept001-modal-form">
              <div className="dept001-modal-field">
                <div className="dept001-modal-label-group">
                  <p className="dept001-modal-label">Nama Departemen <span style={{ color: 'var(--luna-semantic-error)' }}>*</span></p>
                  <p className="dept001-modal-hint">Pastikan jabatan telah sesuai. Contoh: Marketing, Accountant, dll</p>
                </div>
                <input className="dept001-modal-input" placeholder="Isi Nama Departemen" value={namaInput} onChange={e => setNamaInput(e.target.value)} />
              </div>
              <div className="dept001-modal-field">
                <div className="dept001-modal-label-group">
                  <p className="dept001-modal-label">Deskripsi <span style={{ color: '#9aa3b0', fontWeight: 400, marginLeft: 4 }}>(Opsional)</span></p>
                </div>
                <textarea className="dept001-modal-textarea" placeholder="Masukan Deskripsi Singkat" value={deskripsiInput} onChange={e => setDeskripsiInput(e.target.value)} />
              </div>
            </div>
            <div className="dept001-modal-footer dept001-modal-footer-end">
              <button className="dept001-modal-btn-cancel" onClick={closeAddModal}>Batal</button>
              <button className="dept001-modal-btn-primary" onClick={handleSimpan}>Simpan</button>
            </div>
          </div>
        </div>
      )}
