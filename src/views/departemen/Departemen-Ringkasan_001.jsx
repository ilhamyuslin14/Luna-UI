...
export default function DepartemenRingkasan_001({ departemen = 'Human Resource', departemenId }) {
  ...
  const loadData = async () => {
    if (!companyId || (!departemen && !departemenId)) return;
    setIsLoading(true);
    try {
      const data = departemenId
        ? await getDepartmentById(departemenId)
        : await getDepartmentByName(companyId, departemen);
      if (data) {
  ...
  return (
    <div className="sd001-columns" style={{ width: '569px', maxWidth: '100%' }}>
      <div className="sd001-col-left" style={{ width: '100%', flex: 'none' }}>

        {/* ── Details Card ── */}
        <div className="sd001-card">
          <div className="sd001-card-header">
            <span className="sd001-card-title" style={{ textTransform: 'uppercase', letterSpacing: '0.65px', fontSize: '13px' }}>
              Detail
            </span>
          </div>
          ...
          {isEditingDesc ? (
            ...
            <div className="sd001-deskripsi-editor">
              ...
              <div className="sd001-detail-edit-footer">
                <button className="sd001-edit-cancel-btn" onClick={handleCancelDesc}>Batal</button>
                <button className="sd001-edit-save-btn" onClick={handleSaveDesc}>Simpan</button>
              </div>
            </div>
          ) : (
            /* ── View mode ── */
            descHtml ? (
              <div
                className="sd001-deskripsi-content sd001-deskripsi-content--hoverable"
