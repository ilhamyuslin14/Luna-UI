    ...
    <div className="kd-view">
      ...
      <div className={`kd-content-wrapper${(activeTab === 'resume' || activeTab === 'seleksi') ? ' kd-content-wrapper--viewer' : ''}`}>
        {activeTab === 'ringkasan' && (
          <div style={{ margin: '-20px -20px 0', padding: '8px 20px', display: 'flex', alignItems: 'center' }}>
            <button className="kd-back-btn" onClick={() => back ? back() : navigate('kandidat')}>
      ...
      {toast && (
        <div className="kd-toast">
          ...
          <div className="kd-toast-text">
            <span className="kd-toast-main">Kandidat berhasil ditambahkan ke posisi</span>
            <span className="kd-toast-sub">Harap tunggu, proses penilaian sedang berlangsung</span>
          </div>
          <button className="kd-toast-close" onClick={() => setToast(false)}>
