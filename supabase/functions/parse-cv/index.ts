...
async function callGeminiForParsing({ cvText, fileBase64, fileMimeType, apiKey, model, prompt, useFlexMode, temperature }) {
  ...
  systemPrompt += '\n\nCRITICAL RULE: Determine if the document is a valid CV/Resume. If it is NOT a valid CV (e.g. a brochure, menu, random article), set is_valid_cv to false, provide the reason in reason_not_valid USING INDONESIAN LANGUAGE (Bahasa Indonesia) IN MAXIMUM 8 WORDS (e.g. "Bukan CV, merupakan brosur produk"), and leave all other fields empty/null. If it is a valid CV, set is_valid_cv to true and extract the information.'
